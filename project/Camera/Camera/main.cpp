...
Camera camera = Camera(glm::vec3(0, 0, -2.0f),glm::vec3(0,0,0),glm::vec3(0,1.0f,0));
...
int main()
{
	...
	Shader myshader = Shader("vertexSource.vert", "fragmentSource.frag");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			float rad = 20.0f * i;
			model = glm::rotate(model, glm::radians(rad), glm::vec3(1.0, 0.3, 0.5));
...
void processInput(GLFWwindow *window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
		camera.keyPress("FORWARD", deltatime);
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
		camera.keyPress("BACKWARD", deltatime);
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
		camera.keyPress("LEFT", deltatime);
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
		camera.keyPress("RIGHT", deltatime);
}
